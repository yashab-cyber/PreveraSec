...
class ConfigManager {
     ...
     */
    static parseConfig(content, filePath) {
    ...
    static applyEnvironmentOverrides(config) {
        ...
        if (process.env.PREVERSEC_OUTPUT_FORMAT) {
            envConfig.output.format = process.env.PREVERSEC_OUTPUT_FORMAT;
        }
        return envConfig;
    }
    /**
    ...
    static validate(config) {
        ...
        if (!config.ingestors)
            errors.push('Missing ingestors configuration');
        if (!config.enrichment)
            errors.push('Missing enrichment configuration');
        if (!config.rag)
            errors.push('Missing RAG configuration');
        if (!config.dast)
            errors.push('Missing DAST configuration');
        // Type validation
